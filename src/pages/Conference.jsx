...
function NotaSelector() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-heading text-3xl sm:text-4xl font-semibold text-[#E8E9FF] tracking-tight">Conferencia</h1>
      <p className="text-zinc-500 text-sm">Selecione uma nota para iniciar a conferencia</p>
      {available.length === 0 ? (
        <div className="bg-[#12134A] border border-[#2D3090]/60 rounded-md p-12 text-center text-zinc-600">
        ...
        <div className="grid gap-3">
          {available.map(nota => (
            <button key={nota.id} onClick={() => navigate(`/conferencia/${nota.id}`)}
              className="bg-[#12134A] border border-[#2D3090]/60 rounded-md p-4 text-left hover:border-blue-500/40 transition-all group">
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-mono text-[#E8E9FF] text-lg">NF-e {nota.numero || '-'}</span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <PlayCircle className="h-5 w-5 text-[#7B84E0] opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              </div>
            </button>
          ))}
...
function ConferenceGame({ notaId }) {
  ...
  if (!isActive && !isDone) {
    return (
      <div className="space-y-6">
        ...
        <div className="bg-[#12134A] border border-[#2D3090]/60 rounded-lg p-16 text-center">
          <ScanBarcode className="h-16 w-16 mx-auto mb-6 text-[#7B84E0]" />
          <h2 className="text-2xl text-[#E8E9FF] font-semibold mb-2">Pronto para conferir</h2>
          ...
          <button onClick={handleStart}
            className="inline-flex items-center gap-3 px-8 py-4 bg-[#2D3090] text-white text-xl font-semibold rounded-lg hover:bg-[#3A42B0] transition-colors">
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-[#12134A] border border-[#2D3090]/60 rounded-lg p-16 text-center">
          ...
          <h2 className="text-3xl text-[#E8E9FF] font-bold mb-2">
          ...
          {nota.operador_conferencia && <p className="text-zinc-400 mb-6 text-sm">Conferido por: <span className="text-[#E8E9FF] font-semibold">{nota.operador_conferencia}</span></p>}
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => navigate(`/relatorio/${notaId}`)}
              className="inline-flex items-center gap-2 px-6 py-3 bg-[#2D3090] text-white rounded-md hover:bg-[#3A42B0] transition-colors font-semibold">
              <Printer className="h-5 w-5" /> Imprimir Relatorio
            </button>
            <button onClick={() => navigate('/conferencia')}
              className="px-6 py-3 bg-[#1E2070] border border-zinc-700 text-zinc-200 rounded-md hover:bg-zinc-800 transition-colors">
  ...
  return (
    <div className="space-y-4" onClick={() => scannerRef.current?.focus()}>
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/conferencia')} className="p-1.5 hover:bg-[#1E2070] rounded transition-colors">
          <ArrowLeft className="h-5 w-5 text-zinc-400" />
        </button>
        <div>
          <h1 className="font-heading text-xl font-semibold text-[#E8E9FF]">NF-e {nota.numero || '-'}</h1>
          <p className="text-zinc-500 text-xs">{nota.fornecedor_nome}</p>
        </div>
        <div className="ml-auto">
          <button onClick={handleFinalize}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm transition-colors ${
              allComplete ? 'bg-green-600 text-white hover:bg-green-500' : 'bg-[#1E2070] text-zinc-400 border border-zinc-800 hover:bg-zinc-800'
            }`}>
      ...
      <div className="bg-[#12134A] border border-[#2D3090]/60 rounded-md p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="font-mono text-lg text-[#E8E9FF] font-bold">{itensCompletos} / {itens.length} itens</span>
        </div>
        <Progress value={progressPct} className={`h-4 bg-[#1E2070] ${progressPct === 100 ? '[&>div]:bg-green-500' : '[&>div]:bg-blue-500'}`} />
      </div>
      ...
      <div className={`bg-[#12134A] border-2 rounded-lg min-h-[380px] flex flex-col items-center justify-center p-8 text-center transition-colors ${
        feedback?.kind === 'erro' ? 'border-red-500/60' : activeItem ? 'border-blue-500/50' : allComplete ? 'border-green-500/60' : 'border-[#2D3090]/60'
      }`}>
        {activeItem ? (
            <div className="w-full max-w-3xl space-y-5">
            <p className="text-[11px] uppercase tracking-[0.2em] text-[#7B84E0]">Codigo Interno</p>
            <p className="font-mono text-6xl sm:text-8xl font-bold text-[#E8E9FF] leading-none">
            ...
            <div className="flex items-end justify-center gap-3">
              <span className="font-mono text-7xl sm:text-8xl font-bold text-[#7B84E0]">
            ...
            <Progress value={pctItem} className="h-5 bg-[#1E2070] [&>div]:bg-blue-500 max-w-xl mx-auto" />
            <div className="pt-3 border-t border-[#2D3090]/60 flex items-center justify-center gap-6 text-zinc-500">
            ...
            <button onClick={(e) => { e.stopPropagation(); handleSkip(); }}
              className="inline-flex items-center gap-2 px-4 py-2 mt-2 bg-[#1E2070] border border-zinc-700 text-zinc-400 rounded-md text-sm hover:bg-zinc-800 hover:text-zinc-200 transition-colors">
          ...
          <div className="space-y-4">
            <ScanBarcode className="h-20 w-20 mx-auto text-[#7B84E0] animate-pulse" />
            <p className="text-3xl sm:text-4xl font-bold text-[#E8E9FF]">BIPE UM PRODUTO</p>
            <p className="text-zinc-500 text-lg">Aponte o leitor para o codigo de barras de qualquer produto da nota</p>
          </div>
        )}
      </div>

      <div className="bg-[#0D0E2A] border border-[#2D3090]/60 rounded-md p-3 flex items-center gap-3">
        <ScanBarcode className="h-5 w-5 text-[#7B84E0] shrink-0" />
          ...
          return (
            <div key={item.id}
              className={`rounded-md border p-2.5 text-left ${
                ...
                'border-[#2D3090]/60 bg-[#12134A]'
              }`}>
              <div className="flex items-center justify-between gap-2">
                <span className="font-mono text-sm text-[#E8E9FF] truncate">{item.produto_interno_codigo}</span>
      ...
      <Dialog open={finalizeDialogOpen} onOpenChange={setFinalizeDialogOpen}>
        <DialogContent className="bg-[#12134A] border-[#2D3090]/60 text-[#E8E9FF] max-w-md">
          <DialogHeader>
          ...
          <div>
            <label className="text-[11px] uppercase tracking-[0.15em] text-[#7B84E0] mb-2 block">Nome do Estoquista</label>
            <input value={operadorNome} autoFocus
              ...
              className="w-full text-lg p-3 bg-black text-white border-2 border-[#4B52C4]/30 rounded-md focus:border-blue-500 focus:outline-none placeholder:text-zinc-700" />
          </div>
          <div className="flex justify-end gap-2 mt-2">
            <button onClick={() => setFinalizeDialogOpen(false)}
              className="px-4 py-2 text-sm text-zinc-400 hover:text-[#E8E9FF] transition-colors">Cancelar</button>
...
function Header({ nota, navigate }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <h1 className="font-heading text-2xl sm:text-3xl font-semibold text-[#E8E9FF] tracking-tight">NF-e {nota.numero || '-'}</h1>
